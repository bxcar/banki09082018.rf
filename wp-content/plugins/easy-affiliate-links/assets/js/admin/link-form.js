...
eafl_admin.clear_link_fields = function() {
    ...
    jQuery('#eafl-link-cloak-default').prop('checked', true).trigger('change');
...
eafl_admin.set_link_fields = function(link) {
    ...
    jQuery('#eafl-link-cloak-' + link.cloak ).prop('checked', true).trigger('change');
...
eafl_admin.insert_update_link = function(button) {
    ...
    var link = {
        ...
        cloak: jQuery('input[name="eafl-link-cloak"]:checked').val(),
...
eafl_admin.convert_to_slug = function(slug) {
    slug = eafl_admin.removeDiacritics(slug);
...
eafl_admin.removeDiacritics = function(str) {
    for(var i=0; i<eafl_admin.defaultDiacriticsRemovalMap.length; i++) {
        str = str.replace(eafl_admin.defaultDiacriticsRemovalMap[i].letters, eafl_admin.defaultDiacriticsRemovalMap[i].base);
    }
    return str;
}

eafl_admin.defaultDiacriticsRemovalMap = [
...
jQuery(document).ready(function($) {
    ...
    jQuery('input[name="eafl-link-cloak"]').on('change', function() {
        var cloak = jQuery('input[name="eafl-link-cloak"]:checked').data('cloak') == 'no' ? false : true;

        if(cloak) {
            jQuery('#eafl-form-line-shortlink-slug').show();
        } else {
            jQuery('#eafl-form-line-shortlink-slug').hide();
        }
    });
